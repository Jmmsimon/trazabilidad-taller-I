...
export default function DocenteLoginPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      if (rol === "profesor") {
        ...
        logout();
        setError("Esta cuenta no está registrada como Docente.");
      }
    }
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 flex items-center justify-center relative overflow-hidden font-sans">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-emerald-650/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-650 rounded-2xl mx-auto flex items-center justify-center mb-4 shadow-lg shadow-emerald-500/20">
            <Sparkles className="w-7 h-7 text-white" />
          </div>
          <h1 className="text-2xl font-bold">Portal de Docentes</h1>
          <p className="text-zinc-400 text-xs mt-1.5">
            Ingresa a tu entorno de supervisión académica
          </p>
        </div>
        ...
        <div className="backdrop-blur-xl bg-zinc-900/40 border border-zinc-800/50 rounded-3xl p-8 shadow-2xl space-y-6">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  type="email"
                  required
                  placeholder="docente@institucion.edu.pe"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full bg-zinc-950/60 border border-zinc-800 rounded-xl py-3 pl-11 pr-4 text-zinc-200 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 placeholder:text-zinc-700"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isSigningIn}
              className="w-full mt-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg disabled:opacity-60 cursor-pointer text-sm"
            >
